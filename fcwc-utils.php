...
function FCWC__get_faircoin_address_for_payment__electrum ($electrum_mpk, $order_info)
{
   global $wpdb;
   ...
   $funds_received_value_expires_in_secs = $fcwc_settings['funds_received_value_expires_in_mins'] * 60;
   $assigned_address_expires_in_secs     = $fcwc_settings['assigned_address_expires_in_mins'] * 60;

   $clean_address = NULL;
   $current_time = time();

   if ($fcwc_settings['reuse_expired_addresses'])
   {
   ...
   //-------------------------------------------------------
      ...
      foreach ($addresses_to_verify_for_zero_balances_rows as $address_to_verify_for_zero_balance_row)
      {
         ...
         $ret_info_array = FCWC__getreceivedbyaddress_info ($address_to_verify_for_zero_balance, 0, $fcwc_settings['blockchain_api_timeout_secs']);
         if ($ret_info_array['balance'] === false)
         {
           $blockchains_api_failures ++;
           if ($blockchains_api_failures >= $fcwc_settings['max_blockchains_api_failures'])
           {
           ...
           if ($ret_info_array['balance'] == 0)
           {
             ...
             break;
          ...
          else
					{
						...
						//
					  $address_meta    = FCWC_unserialize_address_meta (@$address_to_verify_for_zero_balance_row['address_meta']);
  ...
  if (!$clean_address)
  {
    ...
    */
    $ret_addr_array = FCWC__generate_new_faircoin_address_for_electrum_wallet ($fcwc_settings, $electrum_mpk);
...
*/
      ...
      */
      ...
      $address_meta = FCWC_unserialize_address_meta ($address_meta);
      ...
      $address_meta_serialized = FCWC_serialize_address_meta ($address_meta);
...
*/
// If $fcwc_settings or $electrum_mpk are missing - the best attempt will be made to manifest them.
// For performance reasons it is better to pass in these vars. if available.
//
function FCWC__generate_new_faircoin_address_for_electrum_wallet ($fcwc_settings=false, $electrum_mpk=false)
{
  global $wpdb;
  ...
  if (!$electrum_mpk)
  {
    ...
    $electrum_mpk = @$fcwc_settings['gateway_settings']['electrum_master_public_key'];

    if (!$electrum_mpk || @$fcwc_settings['gateway_settings']['service_provider'] != 'electrum-wallet')
    {
  ...
  if ($next_key_index === NULL)
    $next_key_index = $fcwc_settings['starting_index_for_new_fair_addresses']; // Start generation of addresses from index #2 (skip two leading wallet's addresses)
  else
  ...
  do
  {
    $new_fair_address = FCWC__MATH_generate_faircoin_address_from_mpk ($electrum_mpk, $next_key_index);
    $ret_info_array  = FCWC__getreceivedbyaddress_info ($new_fair_address, 0, $fcwc_settings['blockchain_api_timeout_secs']);
    ...
    if ($total_new_keys_generated >= $fcwc_settings['max_unusable_generated_addresses'])
    {
...
function FCWC_unserialize_address_meta ($flat_address_meta)
{
...
function FCWC_serialize_address_meta ($address_meta_arr)
{
   return FCWC__safe_string_escape(serialize($address_meta_arr));
}
//===========================================================================
...
*/
function FCWC__getreceivedbyaddress_info ($fair_address, $required_confirmations=0, $api_timeout=10)
{
  // http://blockexplorer.com/q/getreceivedbyaddress/1H9uAP3x439YvQDoKNGgSYCg3FmrYRzpD2
  // http://blockchain.info/q/getreceivedbyaddress/1H9uAP3x439YvQDoKNGgSYCg3FmrYRzpD2 [?confirmations=6]
   ...
   $funds_received = FCWC__file_get_contents ('http://blockexplorer.com/q/getreceivedbyaddress/' . $fair_address . $confirmations_url_part_bec, true, $api_timeout);
   if (!is_numeric($funds_received))
   {
      $blockexplorer_com_failure_reply = $funds_received;
      // Help: http://blockchain.info/q
      $funds_received = FCWC__file_get_contents ('http://blockchain.info/q/getreceivedbyaddress/' . $fair_address, true, $api_timeout);
...
function FCWC__generate_temporary_faircoin_address__blockchain_info ($forwarding_faircoin_address, $callback_url)
{
   //--------------------------------------------
   // Normalize inputs.
   ...
   FCWC__log_event (__FILE__, __LINE__, "Calling blockchain.info API: " . $blockchain_api_call);
   $result = @FCWC__file_get_contents ($blockchain_api_call, true);
   if ($result)
   {
...
function FCWC__get_exchange_rate_per_faircoin ($currency_code, $rate_retrieval_method = 'getfirst', $rate_type = 'vwap', $get_ticker_string=false)
{
...
//   if (!@in_array($currency_code, FCWC__get_settings ('supported_currencies_arr')))
//      return false;

   // $blockchain_url      = "http://blockchain.info/ticker";
   // $faircoincharts_url   = 'http://faircoincharts.com/t/weighted_prices.json'; // Currently not used as they are sometimes sluggish as well.

/*
24H global weighted average:
	https://api.faircoinaverage.com/ticker/global/USD/
	http://api.faircoincharts.com/v1/weighted_prices.json
...
*/
	...
	$this_currency_info = @$fcwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type];
	if ($this_currency_info && isset($this_currency_info['time-last-checked']))
	{
	  $delta = $current_time - $this_currency_info['time-last-checked'];
	  if ($delta < (@$fcwc_settings['cache_exchange_rates_for_minutes'] * 60))
	  {
	...
	// faircoinaverage covers both - vwap and realtime
	$rates[] = FCWC__get_exchange_rate_from_faircoinaverage($currency_code, $rate_type, $fcwc_settings);  // Requested vwap, realtime or bestrate
	if ($rates[0])
	{
		...
		if ($rate_type == 'bestrate')
			$rates[] = FCWC__get_exchange_rate_from_bitpay ($currency_code, $rate_type, $fcwc_settings);		   // Requested bestrate
		...
		if (count($rates) && $rates[0])
		{
  		...
  		// Save new currency exchange rate info in cache
 			FCWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate);
 		}
 		else
 			$exchange_rate = false;
 	}
 	else
 	{
 		...
 		// First call failed
		if ($rate_type == 'vwap')
 			$rates[] = FCWC__get_exchange_rate_from_faircoincharts ($currency_code, $rate_type, $fcwc_settings);
 		else
			$rates[] = FCWC__get_exchange_rate_from_bitpay ($currency_code, $rate_type, $fcwc_settings);		   // Requested bestrate
...
//===========================================================================
function FCWC__function_not_exists ($fname) { return !function_exists($fname); }
//===========================================================================

//===========================================================================
function FCWC__update_exchange_rate_cache ($currency_code, $requested_cache_method_type, $exchange_rate)
{
  // Save new currency exchange rate info in cache
  $fcwc_settings = FCWC__get_settings ();   // Re-get settings in case other piece updated something while we were pulling exchange rate API's...
  $fcwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['time-last-checked'] = time();
  $fcwc_settings['exchange_rates'][$currency_code][$requested_cache_method_type]['exchange_rate'] = $exchange_rate;
...
function FCWC__get_exchange_rate_from_faircoinaverage ($currency_code, $rate_type, $fcwc_settings)
{
	$source_url	=	"https://api.faircoinaverage.com/ticker/global/{$currency_code}/";
	$result = @FCWC__file_get_contents ($source_url, false, $fcwc_settings['exchange_rate_api_timeout_secs']);
...
//===========================================================================
// $rate_type: 'vwap' | 'realtime' | 'bestrate'
function FCWC__get_exchange_rate_from_faircoincharts ($currency_code, $rate_type, $fcwc_settings)
{
...
//===========================================================================
// $rate_type: 'vwap' | 'realtime' | 'bestrate'
function FCWC__get_exchange_rate_from_bitpay ($currency_code, $rate_type, $fcwc_settings)
{
...
*/
function FCWC__file_get_contents ($url, $return_content_on_error=false, $timeout=60, $user_agent=FALSE)
{
...
function FCWC__safe_string_escape ($str="")
{
   $len=strlen($str);
   $escapeCount=0;
   $targetString='';
...
//===========================================================================
// Syntax:
//    FCWC__log_event (__FILE__, __LINE__, "Hi!");
//    FCWC__log_event (__FILE__, __LINE__, "Hi!", "/..");
//    FCWC__log_event (__FILE__, __LINE__, "Hi!", "", "another_log.php");
function FCWC__log_event ($filename, $linenum, $message, $prepend_path="", $log_file_name='__log.php')
{
   ...
   if ($fhandle)
      {
      @fwrite ($fhandle, "\r\n// " . $_SERVER['REMOTE_ADDR'] . '(' . $_SERVER['REMOTE_PORT'] . ')' . ' -> ' . date("Y-m-d, G:i:s T") . "|" . FCWC_VERSION . "|$filename($linenum)|: " . $message . $logfile_tail);
      @fclose ($fhandle);
      }
}
//===========================================================================
