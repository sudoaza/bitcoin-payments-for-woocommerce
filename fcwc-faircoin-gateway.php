...
//---------------------------------------------------------------------------
add_action('plugins_loaded', 'FCWC__plugins_loaded__load_faircoin_gateway', 0);
//---------------------------------------------------------------------------

//###########################################################################
// Hook payment gateway into WooCommerce

function FCWC__plugins_loaded__load_faircoin_gateway ()
{
	 ...
	 * @author 		Faircoop
	 */
	class FCWC_Faircoin extends WC_Payment_Gateway
	{
		//-------------------------------------------------------------------
	    /**
      ...
      if ( version_compare( WOOCOMMERCE_VERSION, '2.0.0', '>=' ) )
	    ...
	    add_action('woocommerce_thankyou_' . $this->id, array(&$this, 'FCWC__thankyou_page')); // hooks into the thank you page after payment

	    	// Customer Emails
	    add_action('woocommerce_email_before_order_table', array(&$this, 'FCWC__email_instructions'), 10, 2); // hooks into the email template to show additional details

			// Hook IPN callback logic
			if (version_compare (WOOCOMMERCE_VERSION, '2.0', '<'))
				add_action('init', array(&$this, 'FCWC__maybe_faircoin_ipn_callback'));
			else
				add_action('woocommerce_api_' . strtolower(get_class($this)),
array($this,'FCWC__maybe_faircoin_ipn_callback'));

			// Validate currently set currency for the store. Must be among supported ones.
			if (!$this->FCWC__is_gateway_valid_for_use()) $this->enabled = false;
	    }
		//-------------------------------------------------------------------
	     ...
	     * @return bool
	     */
	    function FCWC__is_gateway_valid_for_use(&$ret_reason_message=NULL)
	    {
	    	$valid = true;
	    	...
	    	// Validate connection to exchange rate services
	   		...
	   		{
					$currency_rate = FCWC__get_exchange_rate_per_faircoin ($store_currency_code, 'getfirst', 'bestrate', false);
					if (!$currency_rate)
					{
						$valid = false;
						...
						$fns = array_filter ($fns, 'FCWC__function_not_exists');
						$extra_error_message = "";
						if (count($fns))
	    	...
	    	// Validate currency
	   		...
	   		// $supported_currencies_arr = FCWC__get_settings ('supported_currencies_arr');
	    ...
	    function init_form_fields()
	    {
	    	...
	    	// Assemble currency ticker.
	   		...
	   		if ($store_currency_code == 'FAIR')
				...
				$currency_ticker = FCWC__get_exchange_rate_per_faircoin ($currency_code, 'getfirst', 'bestrate', true);
	    	//-----------------------------------

	    	//-----------------------------------
	    	// Payment instructions
	    	$payment_instructions = '
<table class="fcwc-payment-instructions-table" id="fcwc-payment-instructions-table">
  ...
  <tr class="bpit-table-row">
    ...
    <td class="bpit-td-value bpit-td-value-qr">
      <div style="border:1px solid #FCCA09;padding:5px;margin:2px;background-color:#FCF8E3;border-radius:4px;">
        <a href="faircoin://{{{FAIRCOINS_ADDRESS}}}?amount={{{FAIRCOINS_AMOUNT}}}">
          <img src="https://blockchain.info/qr?data=faircoin://{{{FAIRCOINS_ADDRESS}}}?amount={{{FAIRCOINS_AMOUNT}}}&size=180" style="vertical-align:middle;border:1px solid #888;" />
        </a>
      </div>
    </td>
  </tr>
...
';
	    	...
	    	$this->form_fields = array(
				...
				'faircoin_addr_merchant' => array(
								...
								'description' =>
$this->service_provider!='blockchain.info'?__('Available when Faircoin service provider is set to: <b>Blockchain.info</b>', 'woocommerce'):__( 'Your own faircoin address where you would like the payment to be sent. When customer sends you payment for the product - it will be automatically forwarded to this address by blockchain.info APIs.', 'woocommerce' ),
								'default' => '',
							),
				...
				'exchange_rate_type' => array(
								...
								'type' => 'select',
								'disabled' => $store_currency_code=='FAIR'?true:false,
								'options' => array(
								...
								'default' => 'vwap',
								'description' => ($store_currency_code=='FAIR'?__('<span style="color:red;"><b>Disabled</b>: Applies only for stores with non-faircoin default currency.</span><br />', 'woocommerce'):'') .
...
///!!!
									...
									'	<tr><td>Amount (FAIR): </td><td><div style="border:1px solid #CCC;padding:2px 6px;margin:2px;background-color:#FEFEF0;border-radius:4px;color:#CC0000;">{{{FAIRCOINS_AMOUNT}}}</div></td></tr>' .
...
*/
		...
		{
			$validation_msg = "";
			$store_valid    = $this->FCWC__is_gateway_valid_for_use ($validation_msg);
	    	...
	    	<p>
	    		<?php _e('Allows to accept payments in faircoin. <a href="https://fair-coin.org" target="_blank">Faircoins</a> are fair economy, peer-to-peer, decentralized digital currency that enables instant payments from anyone to anyone, anywhere in the world.',	'woocommerce'); ?>
	    	</p>
	    	<?php
    ...
    public function process_admin_options()
    {
    	...
    	if (isset($_POST) && is_array($_POST))
    	{
	  		...
	  		if (!isset($fcwc_settings['gateway_settings']) || !is_array($fcwc_settings['gateway_settings']))
	  			$fcwc_settings['gateway_settings'] = array();
	    	...
	    	foreach ($_POST as $varname => $varvalue)
	    	{
	    		if (strpos($varname, 'woocommerce_faircoin_') === 0)
	    		{
	    			...
	    			if ($trimmed_varname != 'description' && $trimmed_varname != 'instructions')
	    				$fcwc_settings['gateway_settings'][$trimmed_varname] = $varvalue;
	    		}
	    	}

	  		// Update gateway settings within FCWC own settings for easier access.
		...
		function process_payment ($order_id)
		{
			...
			$exchange_rate = FCWC__get_exchange_rate_per_faircoin (get_woocommerce_currency(), 'getfirst', $this->exchange_rate_type);
			/// $exchange_rate = FCWC__get_exchange_rate_per_faircoin (get_woocommerce_currency(), $this->exchange_rate_retrieval_method, $this->exchange_rate_type);
			if (!$exchange_rate)
			{
			...
			if (get_woocommerce_currency() != 'FAIR')
				...
				$order_total_in_fair = $order_total_in_fair * $this->exchange_multiplier;
  		...
  		$ret_info_array = array();
			...
			if ($this->service_provider == 'blockchain.info')
			{
				...
				$callback_url = trailingslashit (home_url()) . "?wc-api=FCWC_Faircoin&secret_key={$secret_key}&faircoin=1&src=bcinfo&order_id={$order_id}";
// http://www.example.com/?faircoin=1&order_id=74&src=bcinfo
	   		FCWC__log_event (__FILE__, __LINE__, "Calling FCWC__generate_temporary_faircoin_address__blockchain_info(). Payments to be forwarded to: '{$faircoin_addr_merchant}' with callback URL: '{$callback_url}' ...");

	   			// This function generates temporary faircoin address and schedules IPN callback at the same
				$ret_info_array = FCWC__generate_temporary_faircoin_address__blockchain_info ($faircoin_addr_merchant, $callback_url);
            ...
            $ret_info_array = array (
				...
				*/
				$ret_info_array = FCWC__get_faircoin_address_for_payment__electrum ($this->electrum_master_public_key, $order_info);
				$faircoins_address = @$ret_info_array['generated_faircoin_address'];
			}
   		...
   		FCWC__log_event (__FILE__, __LINE__, "     Generated unique faircoin address: '{$faircoins_address}' for order_id " . $order_id);
	     ...
	     * @return void
	     */
		function FCWC__thankyou_page($order_id)
		{
			// FCWC__thankyou_page is hooked into the "thank you" page and in the simplest case can just echo’s the description.
			...
			$order_total_in_fair   = get_post_meta($order->id, 'order_total_in_fair',   true); // set single to true to receive properly unserialized array
	     ...
	     * @return void
	     */
		function FCWC__email_instructions ($order, $sent_to_admin)
		{
		 ...
		 * @return void
		 */
		function FCWC__maybe_faircoin_ipn_callback ()
		{
			// If example.com/?faircoin=1 is present - it is callback URL.
			if (isset($_REQUEST['faircoin']) && $_REQUEST['faircoin'] == '1')
			{
     		FCWC__log_event (__FILE__, __LINE__, "FCWC__maybe_faircoin_ipn_callback () called and 'faircoin=1' detected. REQUEST  =  " . serialize(@$_REQUEST));

				if (@$_GET['src'] != 'bcinfo')
				{
					$src = $_GET['src'];
					FCWC__log_event (__FILE__, __LINE__, "Warning: received IPN notification with 'src'= '{$src}', which is not matching expected: 'bcinfo'. Ignoring ...");
					exit();
				}
				...
				{
     			FCWC__log_event (__FILE__, __LINE__, "Warning: secret_key does not match! secret_key sent: '{$secret_key_sent}'. Expected: '{$secret_key}'. Processing aborted.");
				...
				if ($confirmations >= $this->confirmations)
				{
					...
					// The value of the payment received in satoshi (not including fees).
					// Divide by 100000000 to get the value in FAIR.
					...
					{
						FCWC__process_payment_completed_for_order ($order_id, false);
					}
					else
					{
     				FCWC__log_event (__FILE__, __LINE__, "NOTE: Payment received (for FAIR {$value_in_fair}), but not enough yet to cover the required total. Will be waiting for more. Faircoins: now/total received/needed = {$value_in_fair}/{$paid_total_so_far}/{$order_total_in_fair}");
					}
			    ...
			    // Don't print *ok* so the notification resent again on next confirmation
   				FCWC__log_event (__FILE__, __LINE__, "NOTE: Payment notification received (for FAIR {$value_in_fair}), but number of confirmations is not enough yet. Confirmations received/required: {$confirmations}/{$this->confirmations}");
			    exit();
				}
			}
		}
	...
	add_filter ('woocommerce_payment_gateways', 	'FCWC__add_faircoin_gateway' );

	// Disable unnecessary billing fields.
	/// Note: it affects whole store.
	/// add_filter ('woocommerce_checkout_fields' , 	'FCWC__woocommerce_checkout_fields' );

	add_filter ('woocommerce_currencies', 			'FCWC__add_fair_currency');
	add_filter ('woocommerce_currency_symbol', 		'FCWC__add_fair_currency_symbol', 10, 2);

	// Change [Order] button text on checkout screen.
    /// Note: this will affect all payment methods.
    /// add_filter ('woocommerce_order_button_text', 	'FCWC__order_button_text');
	//-----------------------------------------------------------------------
	 ...
	 * @return array
	 */
	function FCWC__add_faircoin_gateway( $methods )
	{
		$methods[] = 'FCWC_Faircoin';
		return $methods;
	}
	//=======================================================================
	...
	function FCWC__woocommerce_checkout_fields ($fields)
	{
	...
	//=======================================================================
	function FCWC__add_fair_currency_symbol($currency_symbol, $currency)
	{
		switch( $currency )
		{
			case 'FAIR':
	...
	//=======================================================================
 	function FCWC__order_button_text () { return 'Continue'; }
	//=======================================================================
}
//###########################################################################

//===========================================================================
function FCWC__process_payment_completed_for_order ($order_id, $faircoins_paid=false)
{
	...
	if (!get_post_meta($order_id, '_payment_completed', true))
	{
		...
		FCWC__log_event (__FILE__, __LINE__, "Success: order '{$order_id}' paid in full. Processing and notifying customer ...");
