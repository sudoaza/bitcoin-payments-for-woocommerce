...
function FCWC_cron_job_worker ($hardcron=false)
{
  ...
  if (@$fcwc_settings['gateway_settings']['service_provider'] != 'electrum-wallet')
  {
  ...
  $confirmations_required = $fcwc_settings['gateway_settings']['confirmations'];
  ...
  if (is_array($rows_for_balance_check))
  {
  	...
  	foreach ($rows_for_balance_check as $row_for_balance_check)
  	{
		  ...
		  $address_meta    = FCWC_unserialize_address_meta (@$row_for_balance_check['address_meta']);
		  ...
		  $balance_info_array = FCWC__getreceivedbyaddress_info ($row_for_balance_check['fair_address'], $confirmations_required, $fcwc_settings['blockchain_api_timeout_secs']);
        ...
        if ($balance_info_array['balance'] > 0)
        {
          ...
          FCWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Detected non-zero balance at address: '{$row_for_balance_check['fair_address']}, order ID = '{$last_order_info['order_id']}'. Detected balance ='{$balance_info_array['balance']}'.");

          if ($balance_info_array['balance'] < $last_order_info['order_total'])
          {
            FCWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: balance at address: '{$row_for_balance_check['fair_address']}' (FAIR '{$balance_info_array['balance']}') is not yet sufficient to complete it's order (order ID = '{$last_order_info['order_id']}'). Total required: '{$last_order_info['order_total']}'. Will wait for more funds to arrive...");
          }
        }
        else
        {
        ...
        //	Scan '$address_meta['orders']' for first UNPAID order that is exactly matching amount at address.
	        ...
	        FCWC__log_event (__FILE__, __LINE__, "Cron job: NOTE: Full payment for order ID '{$last_order_info['order_id']}' detected at address: '{$row_for_balance_check['fair_address']}' (FAIR '{$balance_info_array['balance']}'). Total was required for this order: '{$last_order_info['order_total']}'. Processing order ...");
	        ...
	        FCWC__process_payment_completed_for_order ($last_order_info['order_id'], $balance_info_array['balance']);
          ...
          //
	        $query =
	          "UPDATE `$fair_addresses_table_name`
	            ...
	            WHERE `id`='$row_id';";
	        $ret_code = $wpdb->query ($query);
	        FCWC__log_event (__FILE__, __LINE__, "Cron job: SUCCESS: Order ID '{$last_order_info['order_id']}' successfully completed.");
...
// 	        if (!$hardcron && $ran_cycles >= $fcwc_settings['soft_cron_max_loops_per_run'])
// 	        {
...
// 	        }
		  ...
		  {
		    FCWC__log_event (__FILE__, __LINE__, "Cron job: Warning: Cannot retrieve balance for address: '{$row_for_balance_check['fair_address']}: " . $balance_info_array['message']);
		  }
		  //..//
		}
	}
  ...
  if ($hardcron)
  {
    ...
    if ($electrum_mpk && @$fcwc_settings['gateway_settings']['service_provider'] == 'electrum-wallet')
    {
      ...
      $query =
        "SELECT COUNT(*) as `total_unused_addresses` FROM `$fair_addresses_table_name`
      ...
      if ($total_unused_addresses < $fcwc_settings['max_unused_addresses_buffer'])
      {
        FCWC__generate_new_faircoin_address_for_electrum_wallet ($fcwc_settings, $electrum_mpk);
      }
    }
  }
  //-----------------------------------------------------
